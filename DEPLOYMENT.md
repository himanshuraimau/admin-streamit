...
### Domain Structure
- **Frontend**: `app.vidrelay.site` - React/Vite admin dashboard
- **Backend API**: `api.vidrelay.site` - Bun/Express REST API
...
Login to your domain registrar (where you purchased `vidrelay.site`) and add these DNS records:
...
**Backend Nginx Config (`api.vidrelay.site`):**
```nginx
server {
    listen 80;
...
**Frontend Nginx Config (`app.vidrelay.site`):**
```nginx
server {
    listen 80;
...
# Should show:
# Certificate Name: app.vidrelay.site
#   Domains: app.vidrelay.site api.vidrelay.site
...
**Backend (`/etc/nginx/sites-available/api.vidrelay.site`):**
```nginx
server {
    server_name api.vidrelay.site;
    ...
    ssl_certificate /etc/letsencrypt/live/app.vidrelay.site/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/app.vidrelay.site/privkey.pem; # managed by Certbot
    include /etc/letsencrypt/options-ssl-nginx.conf; # managed by Certbot
    ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem; # managed by Certbot
}

server {
    if ($host = api.vidrelay.site) {
...
curl -vI https://app.vidrelay.site
```
...
SSL Certs:       /etc/letsencrypt/live/app.vidrelay.site/
```
...
- [ ] DNS A records created (app.vidrelay.site, api.vidrelay.site)
- [ ] SSH access verified
...
- [ ] Application tested (https://app.vidrelay.site)
- [ ] API tested (https://api.vidrelay.site/health)
- [ ] Admin login tested
