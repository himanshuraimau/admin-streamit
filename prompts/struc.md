# streamit Admin Dashboard - Complete Implementation Guide
...
The admin dashboard is a separate application for managing streamit platform operations, including:
- User management
...
export function AdminSidebar() {
  ...
  return (
    <aside className="w-64 bg-zinc-900 border-r border-zinc-800">
      <div className="p-6">
        <h1 className="text-2xl font-bold text-white">streamit</h1>
...
2. **Separate Admin Domain**
   - Host admin panel on subdomain: `admin.streamit.space`
...
async function createAdmin() {
  const adminEmail = 'admin@streamit.space'; // Your admin email
...
# 4. Build and deploy
npm run build
pm2 start dist/index.js --name streamit-admin-api
```
...
# Point admin.streamit.space to this build
```
...
server {
    listen 443 ssl;
    server_name admin-api.streamit.space;
...
server {
    listen 443 ssl;
    server_name admin.streamit.space;
...
```json
{
  "name": "streamit-admin-backend",
  "version": "1.0.0",
  "description": "streamit Admin Backend API",
  "main": "dist/index.js",
  "scripts": {
...
BETTER_AUTH_URL="https://admin-api.streamit.space"

# Frontend URL for CORS
FRONTEND_URL="https://admin.streamit.space"
...
```env
# API URL
VITE_API_URL=https://admin-api.streamit.space

# Better Auth
VITE_BETTER_AUTH_URL=https://admin-api.streamit.space
```
...
async function seedAdmin() {
  ...
  const email = 'admin@streamit.space';
...
This admin dashboard will give you complete control over your streamit platform!
